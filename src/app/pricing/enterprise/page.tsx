...
const PAKET = [
  { id: "1", nama: "Enterprise Starter", harga: "Rp15jt/thn", pengguna: "s/d 50 pengguna", fitur: ["Semua fitur Professional", "SSO & audit log", "SLA 99,9%"] },
  { id: "2", nama: "Enterprise Plus", harga: "Rp35jt/thn", pengguna: "s/d 200 pengguna", fitur: ["Semua Starter", "Server dedicated", "Manajer khusus"] },
  { id: "3", nama: "Enterprise Custom", harga: "Hubungi kami", pengguna: "Tanpa batas", fitur: ["On-premise / VPC", "Kontrak & NDA khusus", "Tim support 24/7"] },
];
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <section className="py-20 bg-zinc-900 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        ...
        <p className="text-lg text-zinc-300">Solusi skala besar dengan jaminan layanan.</p></div></section>
      <section className="py-12 bg-white dark:bg-slate-950"><div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{f.map((p) => (
          <div key={p.id} className="p-6 rounded-2xl border border-zinc-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            <p className="text-sm text-zinc-500 mb-4">{p.pengguna}</p>
            <ul className="space-y-2">{p.fitur.map((ft, i) => (
              <li key={i} className="flex items-start gap-2 text-zinc-600 dark:text-zinc-400"><Check className="h-5 w-5 text-green-500 shrink-0" />{ft}</li>))}</ul>
          </div>))}</div>
