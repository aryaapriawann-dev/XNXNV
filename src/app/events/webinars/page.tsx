...
const DATA = [
  { id: "1", nama: "VPN 101 Pemula", desc: "Dasar keamanan digital.", kategori: "pemula", meta: "15 Jan" },
  { id: "2", nama: "Bisnis Aman", desc: "VPN untuk UMKM.", kategori: "bisnis", meta: "22 Jan" },
  { id: "3", nama: "Deep Dive WireGuard", desc: "Teknis untuk engineer.", kategori: "teknis", meta: "29 Jan" },
  { id: "4", nama: "Tanya Jawab Live", desc: "Jawab semua pertanyaanmu.", kategori: "live", meta: "5 Feb" },
  { id: "5", nama: "Studi Kasus Kampus", desc: "Implementasi 5rb user.", kategori: "edukasi", meta: "12 Feb" }
];
const CATS = [{ id: "all", label: "Semua" }, { id: "pemula", label: "Pemula" }, { id: "bisnis", label: "Bisnis" }, { id: "teknis", label: "Teknis" }, { id: "live", label: "Live" }, { id: "edukasi", label: "Edukasi" }] as const;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <section className="py-20 bg-zinc-900 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        ...
        <p className="text-lg text-zinc-300">Belajar langsung dari para ahli.</p></div></section>
      <section className="py-12 bg-white dark:bg-slate-950"><div className="max-w-6xl mx-auto px-4">
        <div className="relative mb-6"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari webinar..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{f.map((x) => (
          <div key={x.id} className="p-6 rounded-2xl border border-zinc-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <Globe className="h-7 w-7 text-zinc-900 dark:text-white mb-3" />
        ...
        <div className="flex justify-center mt-12"><Link href="/events/register" className="inline-flex items-center gap-2 px-6 py-3 bg-zinc-900 text-white rounded-full font-semibold hover:bg-zinc-700 transition-colors">Daftar Webinar <ChevronRight className="h-5 w-5" /></Link></div>
      </div></section>
    </div>
  );
}
