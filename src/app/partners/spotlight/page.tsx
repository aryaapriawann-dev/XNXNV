...
import { Search, UserCheck, ChevronRight, Check } from "lucide-react";
const DATA = [
  { id: "1", nama: "Netshop ID", desc: "Reseller 2rb lisensi/bulan.", kategori: "reseller", meta: "Gold" },
  { id: "2", nama: "Kampus Merdeka", desc: "100 kampus onboarding.", kategori: "edukasi", meta: "Platinum" },
  { id: "3", nama: "Paygate Indo", desc: "Pembayaran 15 channel.", kategori: "pembayaran", meta: "Silver" },
  { id: "4", nama: "CoWork Hub", desc: "VPN di 30 lokasi.", kategori: "ruang", meta: "Gold" },
  { id: "5", nama: "Gadget Store", desc: "Bundling device+VPN.", kategori: "ritel", meta: "Silver" }
];
const CATS = [{ id: "all", label: "Semua" }, { id: "reseller", label: "Reseller" }, { id: "edukasi", label: "Edukasi" }, { id: "pembayaran", label: "Pembayaran" }, { id: "ruang", label: "Ruang" }, { id: "ritel", label: "Ritel" }] as const;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <section className="py-20 bg-zinc-900 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        ...
        <p className="text-lg text-zinc-300">Mitra terbaik bulan ini.</p></div></section>
      <section className="py-12 bg-white dark:bg-slate-950"><div className="max-w-6xl mx-auto px-4">
        <div className="relative mb-6"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari mitra..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{f.map((x) => (
          <div key={x.id} className="p-6 rounded-2xl border border-zinc-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <UserCheck className="h-7 w-7 text-zinc-900 dark:text-white mb-3" />
