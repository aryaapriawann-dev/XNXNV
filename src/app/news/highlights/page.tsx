...
const DATA = [
  { id: "1", nama: "Server Baru Balikpapan", desc: "Latensi Kalimantan turun 40%.", kategori: "infrastruktur", meta: "6 Des" },
  { id: "2", nama: "Audit Lulus", desc: "Keamanan tersertifikasi.", kategori: "keamanan", meta: "2 Des" },
  { id: "3", nama: "Promo Akhir Tahun", desc: "Diskon 50% semua paket.", kategori: "promo", meta: "1 Des" },
  { id: "4", nama: "Fitur Keluarga", desc: "1 akun 6 anggota.", kategori: "produk", meta: "28 Nov" },
  { id: "5", nama: "Penghargaan 2026", desc: "Startup Digital Terbaik.", kategori: "penghargaan", meta: "25 Nov" }
];
const CATS = [{ id: "all", label: "Semua" }, { id: "infrastruktur", label: "Infrastruktur" }, { id: "keamanan", label: "Keamanan" }, { id: "promo", label: "Promo" }, { id: "produk", label: "Produk" }, { id: "penghargaan", label: "Penghargaan" }] as const;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <section className="py-20 bg-zinc-900 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold mb-4">Sorotan Berita</h1>
        <p className="text-lg text-zinc-300">Kabar terpenting pekan ini.</p></div></section>
      <section className="py-12 bg-white dark:bg-slate-950"><div className="max-w-6xl mx-auto px-4">
        <div className="relative mb-6"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari sorotan..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{f.map((x) => (
          <div key={x.id} className="p-6 rounded-2xl border border-zinc-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <Zap className="h-7 w-7 text-zinc-900 dark:text-white mb-3" />
        ...
        <div className="flex justify-center mt-12"><Link href="/news/trending" className="inline-flex items-center gap-2 px-6 py-3 bg-zinc-900 text-white rounded-full font-semibold hover:bg-zinc-700 transition-colors">Trending <ChevronRight className="h-5 w-5" /></Link></div>
      </div></section>
    </div>
  );
}
